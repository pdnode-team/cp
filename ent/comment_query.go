...
// QueryCp chains the current query on the "cp" edge.
func (_q *CommentQuery) QueryCp() *CPQuery {
...
// WithCp tells the query-builder to eager-load the nodes that are connected to
// the "cp" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *CommentQuery) WithCp(opts ...func(*CPQuery)) *CommentQuery {
	query := (&CPClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if _q.withAuthor != nil || _q.withCp != nil || _q.withParent != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withCp; query != nil {
		if err := _q.loadCp(ctx, query, nodes, nil,
			func(n *Comment, e *CP) { n.Edges.Cp = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *CommentQuery) loadCp(ctx context.Context, query *CPQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *CP)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Comment)
	for i := range nodes {
		if nodes[i].cp_comments == nil {
			continue
		}
		fk := *nodes[i].cp_comments
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "cp_comments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
