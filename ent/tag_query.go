...
func (_q *TagQuery) QueryCps() *CPQuery {
...
func (_q *TagQuery) WithCps(opts ...func(*CPQuery)) *TagQuery {
	query := (&CPClient{config: _q.config}).Query()
...
func (_q *TagQuery) loadCps(ctx context.Context, query *CPQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *CP)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.CpsTable)
		s.Join(joinT).On(s.C(cp.FieldID), joinT.C(tag.CpsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.CpsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.CpsPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "cps" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
